...
#if defined(CONFIG_SPI_RPI_PICO_PIO_HALF_DUPLEX)
/* ------------------- */
/* spi_sio_mode_0_0_tx */
/* ------------------- */
...
#endif /* CONFIG_SPI_RPI_PICO_PIO_HALF_DUPLEX */
...
#else
	if (spi_cfg->operation & SPI_HALF_DUPLEX) {
		LOG_ERR("Half-duplex not enabled");
		return -EINVAL;
	}
#endif /* CONFIG_SPI_RPI_PICO_PIO_HALF_DUPLEX */
...
#else
		LOG_ERR("SIO pin required half-duplex support");
		return -EINVAL;
#endif
	} else {
		/* 4-wire mode */
